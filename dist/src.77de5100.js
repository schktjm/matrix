  ...
  var step = function step(timestamp) {
    ...
    if (timestamp - lastTime > 30) {
      ...
      lines.forEach(function (x) {
        var height = (timestamp - x.delay) / x.v * x.fontSize;
...
var genColor = function genColor() {
  var hsv = {
    h: 102,
    s: 1,
    v: (Math.floor(Math.random() * Math.floor(30)) + 40) / 100
  };
  return conversionHSV2RGB(hsv);
};

var genParams = function genParams() {
  return __spreadArrays(Array(100)).map(function () {
    return {
...
var randChar = function randChar() {
  var mat = 'abcdefghijklmopqrtsuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789   ';
  return mat[Math.floor(Math.random() * mat.length)];
};

var drawOneLine = function drawOneLine(ctx, x, height, fontSize, color) {
  if (color === void 0) {
    color = null;
  }
  ...
  for (var h = 0; h < height; h += fontSize) {
...
var conversionHSV2RGB = function conversionHSV2RGB(hsv) {
  var rgb = {
    r: hsv.v,
    g: hsv.v,
    b: hsv.v
